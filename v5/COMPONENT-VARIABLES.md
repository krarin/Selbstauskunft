...
Dichte / Erscheinungsbild and the phone preview frame. The light/grey/dark and
comfortable/compact mechanics are excluded too — they only re-point the variables below, they are not
...
| **Text input** | Plain · numeric | Empty, filled, hover, focus, focus-visible, invalid, disabled, recessed | **Colour** `--color-text-primary`, `--color-border-default`, `--color-text-disabled` (placeholder), `--color-border-error`, `--color-background-disabled`, `--color-border-disabled`, `--color-border-focus`, `--color-focus-ring`, `--color-background-hover` (recessed) · **Type** `--field-font-size/-line`‡ (default `input.value` 16/24, compact `input.text-md` 14/20), `--font-weight-regular` · **Space** `--field-padding-x`, `--field-padding-y` · **Other** `--field-height`, `--field-radius`‡, `--border-width-default`†, `--border-width-emphasis`†, `--focus-glow`† | |
...
| **Icon button (remove)** | — | Default, hover → danger, focus-visible | **Colour** `--color-text-secondary`, `--color-feedback-error-strong` · **Type** `--field-font-size`‡ · **Space** `--spacing-4` (glyph), `--spacing-6` (hit area) · **Other** `--field-radius`‡ | |
