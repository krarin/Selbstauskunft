...
check('the sidebar mirrors the cards: five steps plus the reference, subs nested inside them',
  ...
  (() => {
    ...
    nested: true,
    // Antragsteller's and Finanzen's parts are always shown; the object's fold, because
    // exactly one of the five applies and it stands alone in the step
    kinds: { antragsteller: ['static'], finanzen: ['static'],
             objekt: ['collapsible'] },
...
check('a step opens and closes its sub-sections with it', await evaluate(`
  (() => {
    // Finanzierungsobjekt is the one step left with folding parts — Antragsteller and
    // Finanzen show theirs outright (see the static check below).
    ...
    count: 5,
    closed: Array(5).fill('false/false'),
    opened: Array(5).fill('true/true'),
    closedAgain: Array(5).fill('false/false'),
  });

/* Antragsteller's four parts and Finanzen's three took the other road: no toggle at
   all, shown outright as soon as the step is open. Nothing to propagate to, so nothing
   to get out of step. */
...
check('a sub-section can be folded away on its own, and reopening the step brings it back',
  ...
  (() => {
    const card = document.querySelector('#objekt');
    const step = card.querySelector(':scope > .card-head > .card-toggle');
    const sub = document.querySelector('#immobilie');
    ...
    const folded = { sub: sub.dataset.open, step: card.dataset.open,
                     others: document.querySelector('#neubau').dataset.open };
